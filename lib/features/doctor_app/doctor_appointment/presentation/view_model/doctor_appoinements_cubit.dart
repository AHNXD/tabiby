...
class DoctorsAppointmentsCubit extends Cubit<DoctorsAppointmentsState> {
  DoctorsAppointmentsCubit(this._doctorsAppointmentRepo)
    : super(DoctorsAppointmentInitial());
  ...
  Future<void> getDoctorAppointments(int? center, String? date) async {
    ...
    final result = await _doctorsAppointmentRepo.getDoctorAppointments(
      center,
      date,
    );
    ...
    result.fold(
      ...
      (doctorsAppointment) => emit(
        DoctorsAppointmentSuccess(doctorsAppointment: doctorsAppointment),
      ),
    );
  }
}
